...
def run_test(listFileName, storeDir):
    ...
    for step in xrange(all_steps):
        np_arr_data, np_arr_label, next_batch_start, _, _ = input_data.read_clip_and_label(
                            listFileName,BATCH_SIZE,1,start_pos=next_batch_start,num_frames_per_clip=CLIP_LENGTH)
        fc6_of_batch = []
        batch_seq_fc6 = sess.run(fc6,feed_dict={images_placeholder:np_arr_data})
        fc6_of_batch.append(batch_seq_fc6)
        fc6_of_batch = np.array(fc6_of_batch)
        fc6_feature_batch = fc6_of_batch.reshape((-1,4096))
