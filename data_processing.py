...
def get_batches(filename, num_classes, batch_index, video_indices, batch_size=10, crop_size=112, channel_num=3, flatten=false):
    lines = open(filename, 'r')
    clips = []
    labels = []
    lines = list(lines)
    for i in video_indices[batch_index: batch_index + batch_size]:
        ...
        if(flatten):
            clips.append(i_clip.reshape((1,crop_size*crop_size*channel_num)))
